...
// NEW FUNCTION: Update overview metrics from table data
function updateOverviewMetricsFromTable() {
    try {
        ...
        let totalNonFulfillmentRate = 0;
        let totalShopRating = 0;
        let totalPenaltyPoints = 0;
        let totalResponseRate = 0;
        let totalHandoverRate = 0;
        let rowCount = 0;
        ...
        tableRows.forEach(row => {
            ...
            // Index 1: Non-fulfillment rate
            // Index 7: Shop rating
            // Index 8: Penalty points
            // Index 5: Response rate
            // Index 4: Fast handover rate
            ...
            // Accumulate values
            totalNonFulfillmentRate += nonFulfillmentRate;
            totalShopRating += shopRating;
            totalPenaltyPoints += penaltyPoints;
            totalResponseRate += responseRate;
            totalHandoverRate += handoverRate;
        ...
        // Calculate averages
        const avgNonFulfillmentRate = rowCount > 0 ? (totalNonFulfillmentRate / rowCount).toFixed(2) : 0;
        const avgShopRating = rowCount > 0 ? (totalShopRating / rowCount).toFixed(1) : 0;
        const avgResponseRate = rowCount > 0 ? (totalResponseRate / rowCount).toFixed(2) : 0;
        const avgHandoverRate = rowCount > 0 ? (totalHandoverRate / rowCount).toFixed(2) : 0;
        
        // Update the overviewMetrics object
        overviewMetrics = {
            non_fulfillment_rate: parseFloat(avgNonFulfillmentRate),
            shop_rating: parseFloat(avgShopRating),
            penalty: totalPenaltyPoints,
            response_rate: parseFloat(avgResponseRate),
            fast_handover_rate: parseFloat(avgHandoverRate)
        };
        
        // Call the existing function to update the UI
        updateOverviewMetrics();
        
        // Update additional metrics if they exist in the UI
...
// NEW FUNCTION: Update additional metrics in the UI
function updateAdditionalMetrics() {
    // Update Response Rate metric if element exists
    const responseRateElement = document.getElementById('responseRate');
    if (responseRateElement) {
        const colorClass = getColorClass(overviewMetrics.response_rate, criteria.response_rate, true);
        responseRateElement.textContent = `${formatValue(overviewMetrics.response_rate, true)}`;
        
        // Update classes
        const classes = Array.from(responseRateElement.classList).filter(cls => !cls.startsWith('text-'));
        responseRateElement.className = [...classes, colorClass].join(' ');
    }
    
    // Update Fast Handover Rate metric if element exists
    const handoverRateElement = document.getElementById('handoverRate');
    if (handoverRateElement) {
        const colorClass = getColorClass(overviewMetrics.fast_handover_rate, criteria.fast_handover_rate, true);
        handoverRateElement.textContent = `${formatValue(overviewMetrics.fast_handover_rate, true)}`;
        
        // Update classes
        const classes = Array.from(handoverRateElement.classList).filter(cls => !cls.startsWith('text-'));
        handoverRateElement.className = [...classes, colorClass].join(' ');
    }
}
...
// UPDATED: Table data function to call updateOverviewMetricsFromTable
function updateTableData() {
    ...
    try {
        ...
        // After updating the table, update the overview metrics from the new data
...
// UPDATED: More reliable color class function
function getColorClass(value, threshold, isHigherBetter = true) {
    if (value === undefined || value === null) return '';
    const numValue = parseFloat(value);
    
    if (isNaN(numValue) || isNaN(threshold)) return '';
