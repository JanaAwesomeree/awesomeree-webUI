...
function initializeCalendar() {
    if (window.flatpickr) {
        ...
        flatpickr("#calendar", {
            ...
            enable: [
                function(date) {
                    // Only enable Monday through Friday of the current week
                    const day = date.getDay();
                    const isWeekday = day >= 1 && day <= 5; // Monday = 1, Friday = 5
                    const isCurrentWeek = date >= monday && date <= friday;
                    return isWeekday && isCurrentWeek;
                }
            ],
            ...
            onReady: function(selectedDates, dateStr) {
                // Set initial date to today if it's a weekday, otherwise to the last valid day
                const today = new Date();
                const dayName = today.toLocaleString('en-US', { weekday: 'long' });
                
                if (today >= monday && today <= friday && apiEndpoints[dayName]) {
                    this.setDate(today);
                } else {
                    // Find the last valid day
                    let lastValidDate = new Date(friday);
        ...
        const scheduleNightlyUpdate = () => {
            ...
            setTimeout(() => {
                const { monday: newMonday, friday: newFriday } = getWeekBounds();
                const calendar = document.querySelector("#calendar")._flatpickr;
                
                // Update calendar bounds
                calendar.set("minDate", newMonday);
                calendar.set("maxDate", newFriday);
