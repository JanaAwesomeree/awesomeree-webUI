...
// Helper function to get appropriate color class
function getColorClass(value, threshold, isHigherBetter) {
    if (isHigherBetter) {
...
// Update table data with performance metrics
function updateTableData() {
    try {
        const tableBody = document.querySelector('#performanceTable tbody');
        if (!tableBody) {
            console.warn('Performance table body not found in DOM');
            return;
        }
        ...
        // Add rows for each shop/seller
        performanceData.forEach(shop => {
            const row = document.createElement('tr');
            
            // Format data for display
            const shopName = shop.shop_name || 'Unknown';
            const nonFulfillRate = shop.non_fulfillment_rate || 0;
            const lateShipRate = shop.late_shipment_rate || 0;
            const prepTime = shop.preparation_time || 0;
            const handoverRate = shop.fast_handover_rate || 0;
            const responseRate = shop.response_Rate || 0;
            const avgResponseTime = shop.average_response_time || 0;
            const shopRating = shop.shopRating || 0;
            const penaltyPoints = shop.penalty_points || 0;
            
            // Add cells with appropriate formatting and color coding
            row.innerHTML = `
                <td>${shopName}</td>
                <td class="${getColorClass(nonFulfillRate, criteria.non_fulfillment_rate, false)}">${nonFulfillRate.toFixed(1)}%</td>
                <td class="${getColorClass(lateShipRate, criteria.late_shipment_rate, false)}">${lateShipRate.toFixed(1)}%</td>
                <td class="${getColorClass(prepTime, criteria.preparation_time, false)}">${prepTime.toFixed(1)}</td>
                <td class="${getColorClass(handoverRate, criteria.fast_handover_rate, true)}">${handoverRate.toFixed(1)}%</td>
                <td class="${getColorClass(responseRate, criteria.response_rate, true)}">${responseRate.toFixed(1)}%</td>
                <td class="${getColorClass(avgResponseTime, criteria.average_response_time, false)}">${avgResponseTime.toFixed(1)}</td>
                <td class="${getColorClass(shopRating, criteria.shop_rating, true)}">${shopRating.toFixed(1)}</td>
                <td class="${getColorClass(penaltyPoints, criteria.penalty_points, false)}">${penaltyPoints}</td>
            `;

            tableBody.appendChild(row);
        });
        
        console.log('Performance table updated with data');
    } catch (error) {
        ...
        if (AUTO_REFRESH_INTERVAL > 0) {
            setInterval(() => {
                ...
                if (selectedDateElement) {
                    ...
                    if (dateStr) {
                        fetchPerformanceData(new Date(dateStr));
                    }
                } else {
                    fetchPerformanceData(new Date());
                }
