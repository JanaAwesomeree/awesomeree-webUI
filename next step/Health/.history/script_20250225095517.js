...
const WeekCalendar = () => {
    ...
    // Format date to display in the selected date message
    const formatSelectedDate = (date) => {
      const options = { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' };
      return date.toLocaleDateString('en-US', options);
    };
    
    // Check if the provided week contains today's date
    const isCurrentWeek = () => {
      const today = new Date();
      const mondayOfToday = getMonday(new Date(today));
      return monday.getDate() === mondayOfToday.getDate() && 
             monday.getMonth() === mondayOfToday.getMonth() && 
             monday.getFullYear() === mondayOfToday.getFullYear();
    };
      ...
      <div className="bg-white rounded-lg shadow-md p-6 max-w-md mx-auto">
          ...
          <div className="flex space-x-2">
            ...
            {!isCurrentWeek() && (
              <button 
                ...
                className="px-3 py-1 bg-blue-100 rounded-md hover:bg-blue-200 text-blue-800"
              >
                Today
              </button>
            )}
        ...
        <div className="grid grid-cols-7 gap-2 mb-4">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, index) => (
            <div key={`header-${index}`} className="text-center font-medium text-gray-700">
              {day}
            </div>
          ))}
        ...
        {selectedDate && (
          <div className="bg-blue-50 p-4 rounded-md text-blue-800 mt-2">
