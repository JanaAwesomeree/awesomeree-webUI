// API configuration
// Function to dynamically generate the URL based on the desired day
function getApiEndpointForDay(day) {
    return `https://t9x8zj0h-8000.asse.devtunnels.ms/shopee-health/date/?day=${day}`;
}
...
function displayDateData(selectedDate) {
    ...
    try {
        ...
        if (!apiEndpoints[dayName]) {
            handleNoDataAvailable(`No data available for ${dayName}`);
        ...
        fetchPerformanceData(dayName).catch(error => {
            ...
            handleNoDataAvailable(`Error loading data for ${dayName}`);
        });
    } catch (error) {
        console.error('Error displaying date data:', error);
        showErrorMessage('Error processing selected date');
        handleNoDataAvailable('Error processing selected date');
    }
}
