    ...
    try {
        ...
        tableBody.innerHTML = performanceData.map(data => `
            <tr>
                ...
                <td class="${getColorClass(data.avg_Response, criteria.avgResponse, false)}">${formatValue(data.average_response_time)}</td>
                <td class="${getColorClass(data.shopRating, criteria.shopRating)}">${formatValue(data.shop_Rating)}</td>
