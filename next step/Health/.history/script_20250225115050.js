...
function displayDateData(selectedDate) {
    ...
    try {
        ...
        // Calculate the past week starting from the selected day
        const { monday } = getWeekBounds(selectedDate);

        // Fetch data for each day in the past week
        for (let i = 0; i < 6; i++) {
            const date = new Date(monday);
            date.setDate(monday.getDate() + i);

            const dayFullName = getDayNameFromDate(date);
            fetchPerformanceData(dayFullName).catch(error => {
                console.error('Error fetching data:', error);
                showErrorMessage(`Failed to fetch data for ${dayFullName}: ${error.message}`);
            });
        }
