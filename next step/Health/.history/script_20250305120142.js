...
const apiEndpoints = {
  ...
  function getWeekBounds(referenceDate = new Date()) {
    ...
    // Calculate Monday for the current week (if Sunday, go back 6 days)
  ...
  async function fetchPerformanceData(day = 'Monday', retryCount = RETRY_COUNT, retryDelay = RETRY_DELAY) {
    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
      try {
        ...
        // Process data (supports both array and object responses)
  ...
  // Calculate Overview Metrics from the performance data
  ...
  function initializeWeeklyCalendar() {
    ...
    todayBtn.addEventListener('click', function () {
      ...
      if (todayEl) {
        // Remove previous selection and mark today as selected
  ...
  function updateCalendar(referenceDate) {
    const { monday } = getWeekBounds(referenceDate);
    ...
    if (calendarDays) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        // Instead of disabling based solely on today's day,
        // enable click if the API endpoint exists for that day.
        if (apiEndpoints[dayFullName]) {
      ...
      // Auto-select today's date if it exists in the current week,
      // otherwise select the first enabled day.
      const today = new Date();
      ...
      if (todayEl && !todayEl.classList.contains('disabled')) {
        ...
        if (firstEnabledDay) {
          ...
          displayDateData(new Date(firstEnabledDay.getAttribute('data-date')));
        }
      }
    }
  ...
  // Sidebar and Page Navigation Functions
  ...
  // Event Listener for dynamically added calendar days (if needed)
  ...
  document.addEventListener('DOMContentLoaded', function() {
    try {
      ...
      // Auto-refresh every 5 minutes for today's performance data
      setInterval(() => {
